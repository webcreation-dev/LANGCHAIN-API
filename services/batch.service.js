...
questionQueue.on('completed', (job, result) => {
  console.log(
    `Job ${job.id} complété - ${result.totalQuestionsGenerated} questions`
  );
});
...
export async function createBatches(documentId, config) {
  try {
    ...
    console.log(
      `Document avec ${totalChunks} chunks, création de ${totalBatches} batches`
    );

    for (let batchNumber = 0; batchNumber < totalBatches; batchNumber++) {
      const chunkIndexStart = batchNumber * maxBatchSize;
      const chunkIndexEnd = Math.min(
        (batchNumber + 1) * maxBatchSize - 1,
        totalChunks - 1
      );
      ...
      console.log(
        `Batch ${batchNumber} créé et job ${job.id} ajouté à la queue`
      );
    }
...
export function setupQueueWorkers(concurrency = 5) {
  ...
  const concurrencyInt = Math.floor(concurrency);

  questionQueue.process('__default__', concurrencyInt, async job => {
    const {
      batchId,
      documentId,
      chunkIndexStart,
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        console.log(
          `Génération questions pour chunk ${i + 1}/${chunks.length}`
        );
        ...
        console.log(`Questions générés: ${questions.length}`);
      ...
      console.log(
        `✅ Batch ${batchId} complété: ${totalQuestionsGenerated} questions`
      );
