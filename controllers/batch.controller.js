...
export async function startBatchGeneration(req, res) {
  try {
    ...
    res.json({
      ...
      estimatedQuestions:
        batches.length * parseInt(maxBatchSize) * parseInt(questionsPerChunk),
...
export async function getAllQuestionsForDocumentHandler(req, res) {
  try {
    const { documentId } = req.params;

    // Récupérer toutes les questions associées au document
    const questions = await Question.find({ documentId }).sort({
      createdAt: 1,
    });
    ...
    console.error('Erreur dans getAllQuestionsForDocumentHandler:', error);
    res.status(500).json({ error: 'Erreur interne du serveur' });
  }
}
